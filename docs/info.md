...
Release external drive on `uio[0]` during the 80,000-clock (1 ms) wait after configuration. Its output enable then changes from input to output automatically; no handshake is needed. The first valid frame is area (`uio[0]=0`), followed by peak (`uio[0]=1`), alternating every clock. Read the 11-bit value as `{uio[7:5], uo[7:0]}` after each edge. A coincident sample is included in that frame. Before handoff, the data pins show area and `uio[0]` is an input.
