...
class Reference:
    ...
    def edge(self, reset, level, adc):
        ...
        self.kind = int(ready_before and not self.kind)
    ...
    def sample(self, adc):
        ...
        if self.ready and not self.kind and self.peak != old_peak:
            self.cover['same_edge_new_peak'] += 1
        self.samples += 1
        if self.samples in (1, 1024, 2047, 2048, 2049):
            self.cover[f'sample_{self.samples}'] += 1
