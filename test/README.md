...
Reset is held for four edges. Only the first power-on reset edge allows unsettled low data bits; every later edge rejects unknown outputs, including subsequent resets. The manual `gate-validation` workflow reuses the completed 6x2 GDS after checking that RTL, clock configuration, and tiles are unchanged.
